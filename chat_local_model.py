#!/usr/bin/env python3
"""Chat IA local sans API avec entraînement continu, persistance et turbo-quantization."""
...
class TurboQuantNgramModel:
    """Modèle n-gram léger avec apprentissage incrémental et turboquant."""
    ...
    def _distribution(self, context: Tuple[str, ...]) -> Dict[str, float]:
        ...
        return dict(self.counts.get(context, Counter()))

    def generate(self, prompt: str, max_new_tokens: int = 60, temperature: float = 0.9) -> str:
        base = self.tokenize(prompt) or ["bonjour"]
        generated: List[str] = []

        for _ in range(max_new_tokens):
            context = tuple((base + generated)[-(self.order - 1) :])
    ...
    @classmethod
    def load(cls, path: Path, seed: int = 7) -> "TurboQuantNgramModel":
        if path.suffix == ".gz":
            with gzip.open(path, "rt", encoding="utf-8") as f:
                raw = json.loads(f.read())
        else:
            raw = json.loads(path.read_text(encoding="utf-8"))
        model = cls(order=raw.get("order", 3), turboquant=raw.get("turboquant", True), seed=seed)
        counts = raw.get("counts", {})
        for ctx_str, values in counts.items():
            ctx = tuple(ctx_str.split("\t"))
            model.counts[ctx] = Counter(values)
            if model.turboquant:
                model._quantize_recent(list(ctx) + list(values.keys()))
        return model
...
def train_on_wikipedia(model: TurboQuantNgramModel, wiki_path: Path, limit_docs: int | None) -> int:
    updates = 0
    for doc in load_wikipedia_corpus(wiki_path, limit_docs):
...
def web_search(query: str, max_results: int = 5, timeout: float = 10.0) -> List[SearchResult]:
    ...
    for block in blocks[: max_results * 2]:
        ...
        result = SearchResult(title=clean(t.group(2)), snippet=clean(s.group(1)), url=t.group(1))
...
def summarize_results(query: str, results: List[SearchResult], max_sentences: int = 4) -> str:
    ...
    for r in results:
        ...
        scored.append((overlap / max(1, math.sqrt(len(toks))), sentence))

    scored.sort(key=lambda x: x[0], reverse=True)
    return " ".join(s for _, s in scored[:max_sentences])


def build_reply(local_generation: str, web_summary: str | None) -> str:
    if web_summary:
        return f"Résumé web: {web_summary}\n\nRéponse locale: {local_generation}"
    return local_generation


def create_model(args: argparse.Namespace) -> TurboQuantNgramModel:
    if args.model_in:
        model_path = Path(args.model_in)
        if model_path.exists():
            return TurboQuantNgramModel.load(model_path, seed=args.seed)
    return TurboQuantNgramModel(order=args.order, turboquant=not args.disable_turboquant, seed=args.seed)


def run_training_only(args: argparse.Namespace) -> int:
    ...
    if args.wikipedia_dump:
        ...
        print(f"[train] Wikipedia: {updates} updates en {time.time() - t0:.1f}s")

    if args.model_out:
        model.save(Path(args.model_out))
        print(f"[train] Modèle sauvegardé: {args.model_out}")
...
def chat(args: argparse.Namespace) -> None:
    ...
    if args.wikipedia_dump and not args.skip_wiki_at_start:
        wiki_path = Path(args.wikipedia_dump)
        if not wiki_path.exists():
            raise FileNotFoundError(f"Fichier Wikipedia introuvable: {wiki_path}")
        t0 = time.time()
        updates = train_on_wikipedia(model, wiki_path, args.max_wiki_docs)
        print(f"[init] Entraînement Wikipedia: {updates} updates en {time.time() - t0:.1f}s")

    print("Chat prêt. /quit pour quitter, /web <requête> pour recherche+résumé+entraînement.")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
        while True:
            ...
            web_query = user_msg[5:].strip() if user_msg.startswith("/web ") else None
            ...
            assistant_msg = build_reply(local_generation, web_summary)
            model.train_text(assistant_msg)

            updates = train_future.result(timeout=2)
            print(f"Assistant: {assistant_msg}\n")
            print(f"[learn] +{updates} updates | turboquant={'on' if model.turboquant else 'off'}\n")

    if args.model_out:
        model.save(Path(args.model_out))
        print(f"[save] Modèle sauvegardé: {args.model_out}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="IA chat locale sans API avec apprentissage continu.")
    p.add_argument("--wikipedia-dump", help="Chemin vers dump texte/.jsonl Wikipedia")
    p.add_argument("--max-wiki-docs", type=int, default=2000)
    ...
    p.add_argument("--disable-turboquant", action="store_true")
    p.add_argument("--skip-wiki-at-start", action="store_true")
    p.add_argument("--model-in", help="Charge un modèle .json/.json.gz existant")
    p.add_argument("--model-out", help="Sauvegarde le modèle .json/.json.gz en sortie")
    p.add_argument("--train-only", action="store_true", help="Mode entraînement puis sauvegarde")
    return p.parse_args()
