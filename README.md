...
- Sauvegarde/chargement du modèle (`--model-out`, `--model-in`), y compris format compressé `.json.gz`.

### Lancement local
```bash
python3 chat_local_model.py --model-out ./models/turbo_local_model.json.gz
```
...
python3 chat_local_model.py \
  ...
  --max-wiki-docs 5000 \
  --model-out ./models/turbo_local_model.json.gz
```
...
4. Publie les artifacts:
   ...
   - `models/turbo_local_model.json.gz`
